...
// Autoplay Video Handler for the video section
class AutoplayVideoHandler {
  constructor() {
    this.init();
  }
  ...
  init() {
    ...
    if (document.readyState === 'loading') {
      document.addEventListener('DOMContentLoaded', () => this.handleAutoplayVideos());
    } else {
      this.handleAutoplayVideos();
    }
  }

  handleAutoplayVideos() {
    // Handle Shopify-hosted videos
    const videos = document.querySelectorAll('.video-section__video');
    videos.forEach(video => {
      this.setupVideoAutoplay(video);
    });

    // Handle YouTube iframes
    const youtubeIframes = document.querySelectorAll('.video-section__iframe.js-youtube');
    youtubeIframes.forEach(iframe => {
      this.setupYouTubeAutoplay(iframe);
    });

    // Handle Vimeo iframes
    const vimeoIframes = document.querySelectorAll('.video-section__iframe.js-vimeo');
    vimeoIframes.forEach(iframe => {
      this.setupVimeoAutoplay(iframe);
    });
  }
  ...
  setupVideoAutoplay(video) {
    ...
    if (playPromise !== undefined) {
      playPromise.catch(error => {
        ...
        // If autoplay fails, we could show a play button, but for this use case
        // we want silent failure since the requirement is autoplay only
      });
    }
  ...
  setupYouTubeAutoplay(iframe) {
    // YouTube autoplay is handled via URL parameters
    // Make sure the iframe is loaded
    iframe.addEventListener('load', () => {
      // YouTube API could be used here for more control if needed
      console.log('YouTube video loaded with autoplay');
    });
  }

  setupVimeoAutoplay(iframe) {
    // Vimeo autoplay is handled via URL parameters
    // Make sure the iframe is loaded
    iframe.addEventListener('load', () => {
      // Vimeo API could be used here for more control if needed
      console.log('Vimeo video loaded with autoplay');
    });
  }
}

// Initialize autoplay video handler
if (!window.autoplayVideoHandler) {
  window.autoplayVideoHandler = new AutoplayVideoHandler();
}
